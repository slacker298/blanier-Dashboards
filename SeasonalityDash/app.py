...
#### START SEASOANLITY CODE ####
def plot_seasonality(df,pdf, sym, year1,year2):
    # Create figure with secondary y-axis
    fig = make_subplots(specs=[[{"secondary_y": True}]])

    # Add traces
    fig.add_trace(
        go.Scatter(x=df['trading_day'], y=df['year_change'], name="past ({y1}-{y2})".format(y1=year1, y2=year2)),
        secondary_y=True,
    )

    fig.add_trace(
        go.Scatter(x=pdf['trading_day'], y=pdf['year_change'], hovertext=pdf['date'], name="2021"),
        secondary_y=False,
    )
    # Add figure title
    fig.update_layout(
    title={
        'text': "Seasonality for {}".format(sym),
        'y':0.9,
        'x':0.5,
        'xanchor': 'center',
        'yanchor': 'top'},
    ...
    # Set y-axes titles
    fig.update_yaxes(title_text="<b>present return</b>", secondary_y=False)
    fig.update_yaxes(title_text="<b>past average return</b>", secondary_y=True)
...
def pull_seasonality_data(tda_api, sym, start_year, end_year, exclude_years=None):

    df = get_historical_stock_data(tda_api, sym, '{}-01-01'.format(start_year), '{}-12-31'.format(end_year), frequencyType='daily')

    df['year'] = df['date'].apply(lambda x: x[0:4])
    df['monthdate'] = df['date'].apply(lambda x: x[5:10])

    first_df = df.groupby('year',as_index=False).first()
    first_df = first_df[['year','open']]
    first_df.columns = ['year','year_open']

    df2 = df.merge(first_df, on='year', how='left')

    if exclude_years != None:
        df2 = df2.loc[~df2['year'].isin(exclude_years)]
    # print(df2.shape)
    df2['trading_day1'] = 1
    df2['trading_day'] = df2.groupby('year')['trading_day1'].cumsum()
    df2['year_change']  = ((df2['close'] - df2['year_open']) / df2['year_open'])

    df3 = df2.groupby('trading_day',as_index=False).mean()
    df3 = df3.loc[df3['trading_day']<=251]

    # comparison df time
    pdf = get_historical_stock_data(tda_api, sym, '2021-01-01', '2021-12-31', frequencyType='daily')
    pdf['year'] = pdf['date'].apply(lambda x: x[0:4])
    pdf['year_open'] = pdf.at[0,'open']
    pdf['year_change']  = ((pdf['close'] - pdf['year_open']) / pdf['year_open'])
    pdf['trading_day1'] = 1
    pdf['trading_day'] = pdf.groupby('year')['trading_day1'].cumsum()

    rdf = plot_seasonality(df3,pdf, sym, start_year, end_year)
    return rdf
#### END SEASONALITY CODE ####
...
app.layout = html.Div([
    ...
    html.H1(
        children="Lanier's Seasonality Dashboard",
        style={
            'textAlign': 'center'
        }
    ),
    ...
    html.Div([
        dcc.Markdown(d("""
            ...
            """)),
    dcc.Input(
            id="start-year", type="number",value=1990,
    ...
    html.Div([
        dcc.Markdown(d("""
            ...
            """)),
    dcc.Input(
            id="end-year", type="number",value=2019,
            min=1900, max=2020, step=1,
        ),
    ],style={'width': '49%', 'display': 'inline-block'}),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose years to exclude:
            """)),
        dcc.Dropdown(
            id='year-exclude-dropdown',
            options=[{'label': str(s), 'value': str(s)} for s in range(1900,2021)],
            value=['2008','2009'],
            multi=True
        ),
    ]),
...
@app.callback(
    dash.dependencies.Output('seasonality-graph', 'figure'),
    ...
    dash.dependencies.Input('year-exclude-dropdown', "value")])
def update_figure(sym, start_year, end_year, exclude_years):

    sgraph = pull_seasonality_data(t1,sym,start_year,end_year, exclude_years=exclude_years)
    return sgraph
