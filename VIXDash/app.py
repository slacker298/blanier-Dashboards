...
################## START historical volatility CODE ##################
def calc_Rt(close, lag_close):
...
def realized_vs_vix(tda_api, start_dt='2000-01-01', end_dt=None, n=30, offset=0):

    if end_dt == None:
        todays_date = str(datetime.datetime.now())[0:10]
        end_dt = todays_date

    df = get_historical_stock_data(tda_api, '$SPX.X', start_dt, end_dt, frequencyType='daily')

    df['lag_close'] = df['close'].shift()
    df['R_t'] = df.apply(lambda row: calc_Rt(row['close'],row['lag_close']),axis=1)
    df['R_t2'] = df['R_t'].apply(lambda x: x**2)
    df['rvol1'] = df['R_t2'].rolling(n).sum()
    df['realized_vol'] = df['rvol1'].apply(lambda x: rvol_calc(x))

    vdf = get_historical_stock_data(tda_api, '$VIX.X',start_dt, end_dt, frequencyType='daily')

    try:
        vdf2 = get_historical_stock_data(tda_api, 'VXX', start_dt, end_dt, frequencyType='daily')
        d = {'Realized Vol':df['realized_vol'].values, 'VIX':vdf['close'].values, 'VXX':vdf2['close'].values, 'date':df['date'].values, 'SPX':df['close'].values}
        gdf = pd.DataFrame(d)
    except:
        d = {'Realized Vol':df['realized_vol'].values, 'VIX':vdf['close'].values, 'date':df['date'].values, 'SPX':df['close'].values}
        gdf = pd.DataFrame(d)

    gdf['Realized Vol'] = gdf['Realized Vol'].shift(offset)

    gdf['VIX-HV'] = (gdf['VIX'] - gdf['Realized Vol']) / gdf['Realized Vol']
    gdf['VIX-HV'] = gdf['VIX-HV'].apply(lambda x: round(100*x,2))

    # gdf = gdf.loc[gdf['date']>='2020-01-01']

    return gdf

def plot_realized_vs_vix(gdf):

    fig = make_subplots(specs=[[{"secondary_y": True}]])

    fig.add_trace(go.Scatter(x=gdf['date'], y=gdf['SPX'],
                                line=dict(dash='dot', width=2),
                                name='SPX'),
                    secondary_y=False)
    fig.add_trace(go.Scatter(x=gdf['date'], y=gdf['Realized Vol'],
                                line=dict(width=4), opacity=0.7, visible='legendonly',
                                name='Realized Volatlity'),
                    secondary_y=True)
    fig.add_trace(go.Scatter(x=gdf['date'], y=gdf['VIX'],
                                line=dict(width=2), opacity=0.7, visible='legendonly',
                                name='VIX'),
                    secondary_y=True)

    fig.add_trace(go.Scatter(x=gdf['date'], y=gdf['VIX-HV'],
                            line=dict(width=2, color='#ff7f0e'), opacity=0.8,
                            name='VIX-HV (%)'),
                secondary_y=True)

    try:
        fig.add_trace(go.Scatter(x=gdf['date'], y=gdf['VXX'],
                                    line=dict(width=1, color='#9467bd'), opacity=0.5, visible='legendonly',
                                    name='VXX'),
                        secondary_y=True)
    except:
        pass
    ...
    fig.update_layout(legend_orientation="h", height=1000, title={'text':'Realized Volatility vs. VIX'},xaxis={'title':'date'}, yaxis={'title':'SPX ($)'}, yaxis2={'title':'Volatility'})
    fig.update_layout(legend=dict(x=-.1, y=1.2))

    return {'data': fig.data,
        'layout': fig.layout}
################## END historical volatility CODE ##################
...
app.layout = html.Div([
    ...
    html.H1(
        children="Lanier's VIX Dashboard",
        style={
            'textAlign': 'center'
        }
    ),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose dates for VIX vs. Realized Vol Graph:
            """)),
    dcc.DatePickerSingle(
        id='hv-date1',
        min_date_allowed=datetime.datetime(1986, 1, 1),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose the number of days to use for historical volatility:
            """)),
    dcc.Input(
            id="vix-rolling", type="number",placeholder=20,
            min=2, step=1,
        ),
    ],style={'width': '49%', 'display': 'inline-block'}),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose the number of days to offset historical volatility:
            """)),
    dcc.Input(
            id="vix-offset", type="number",placeholder=0,
            min=0, step=1,
        ),
    ],style={'width': '49%', 'display': 'inline-block'}),

    html.Div([
    dcc.Graph(id='hv-graph') ]),
...
@app.callback(
    dash.dependencies.Output('hv-graph', 'figure'),
    [dash.dependencies.Input('hv-date1', "date"),
    dash.dependencies.Input('hv-date2', "date"),
    dash.dependencies.Input('vix-rolling', "value"),
    dash.dependencies.Input('vix-offset', "value")])
def update_dix(d1, d2, vrolling, voffset):
    ...
    print(d1, d2, vrolling, voffset)

    vixdf = realized_vs_vix(t1, start_dt=d1, end_dt=d2, n=vrolling, offset=voffset)

    vixgraph = plot_realized_vs_vix(vixdf)
