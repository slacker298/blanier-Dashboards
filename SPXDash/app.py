...
def graph_options_oi(tda_api, symbol, start_date, end_date, modifier=None, stack_bars=False):

    df_ = tda_api.get_option_chain_df({'symbol':symbol, 'fromDate':start_date, 'toDate':end_date})

    if modifier in ['delta','gamma','theta','vega']:
        df_ = df_[pd.to_numeric(df_[modifier], errors='coerce').notnull()]
        df_[modifier] = df_[modifier].astype(float)
        df_['openInterest'] = df_['openInterest']*df_[modifier]
        df_['totalVolume'] = df_['totalVolume']*df_[modifier]
...
app.layout = html.Div([
    ...
    html.H1(
        children="Lanier's SPX Dashboard",
        style={
            'textAlign': 'center'
        }
    ),
    ...
    dcc.DatePickerSingle(
        ...
        max_date_allowed=datetime.datetime(2022, 5, 31),
        date=default_date
        )
    ],style={'width': '49%', 'display': 'inline-block'}),
    ...
    html.Div([
    dcc.Graph(id='options2-graph') ]),
    #
    # html.Div([
    # dcc.Graph(id='maxpain-graph') ]),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose number to round contracts to:
            """)),
    dcc.Input(
            id="round-num-val", type="number",value=5,
            min=0, max=25, step=5,
        ),
    ],style={'width': '24%', 'display': 'inline-block'}),
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose money-ness range:
            """)),
    dcc.Input(
            id="moneyness-val", type="number",value=200,
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose how far expirations to include:
            """)),
        dcc.Dropdown(
            id='option-time-dropdown',
            options=[
                    {'label': '1 Month', 'value': 2.628e+6},
                    {'label': '3 Months', 'value': 7.884e+6},
                    {'label': '6 Months', 'value': 1.577e+7},
                    {'label': '1 Year', 'value': 3.154e+7},
                    {'label': '3 Years', 'value': 9.461e+7},
                    ],
            value=2.628e+6
        ),
    ],style={'width': '24%', 'display': 'inline-block'}),
    ...
    html.Div([
    dcc.Graph(id='vts-graph')]),

    ## Date range options graph

    html.Div([
        dcc.Markdown(d("""
                Choose a starting expiration date:
            """)),
    dcc.DatePickerSingle(
        id='start-date',
        min_date_allowed=min_date,
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose a ending expiration date:
            """)),
    dcc.DatePickerSingle(
        id='end-date',
        min_date_allowed=min_date,
    ...
    html.Div([
    dcc.Graph(id='options3-graph') ]),

### DIX AND GEX GRAPH ###
    html.Div([
        dcc.Markdown(d("""
                Choose a starting view date for DIX and GEX:
            """)),
    dcc.DatePickerSingle(
        id='dix-date',
    ...
    html.Div([
        dcc.Markdown(d("""
                Choose number of days to rolling average DIX and GEX:
            """)),
    dcc.Input(
            id="gex-rolling", type="number",placeholder=1,
            min=1, max=100, step=1,
        ),
    ],style={'width': '49%', 'display': 'inline-block'}),

    html.Div([
    dcc.Graph(id='dgx-graph') ]),

    ### VIX values ###
    html.Div([
        dcc.Markdown(d("""
                Calculated VIX using previous, current, and next week SPX contracts
            """)),
            html.Button('update', id='update-vix-button', n_clicks=0)
            ]),
    html.Div([
        html.H2(id='vix0')
        ]),
...
@app.callback(
    dash.dependencies.Output('resistance-graph', 'figure'),
    [dash.dependencies.Input('symbol-dropdown', "value"),
    dash.dependencies.Input('est-res', "value")])
def update_figure(sym, plot_res):

    date = str(datetime.datetime.now())[0:10]
    date2 = str(ciso8601.parse_datetime(datetime.datetime.utcfromtimestamp(time.mktime(datetime.datetime.strptime(date, "%Y-%m-%d").timetuple())-604800).strftime('%Y-%m-%d')))[0:10]
    rgraph = get_resistance_lvls(t1, date2, str(datetime.datetime.now())[0:10], plot_res=plot_res, sym=sym)
    return rgraph

@app.callback(
    [dash.dependencies.Output('options1-graph', 'figure'),
    dash.dependencies.Output('options2-graph', 'figure')],
    [dash.dependencies.Input('symbol-dropdown', "value"),
    dash.dependencies.Input('date-picker', "date"),
    dash.dependencies.Input('3d-plot', "value"),
    dash.dependencies.Input('stack-bars1', "value"),
    dash.dependencies.Input('moneyness-val', "value"),
    dash.dependencies.Input('option-time-dropdown', "value"),
    dash.dependencies.Input('round-num-val', "value")])
def update_figure(sym, date, dplot, stack_bars, lim, opt_t, round_num):

    date = str(date)[0:10]
    # date2 = str(ciso8601.parse_datetime(datetime.datetime.utcfromtimestamp(time.mktime(datetime.datetime.strptime(date, "%Y-%m-%d").timetuple())-604800).strftime('%Y-%m-%d')))[0:10]
    date3 = str(ciso8601.parse_datetime(datetime.datetime.utcfromtimestamp(time.mktime(datetime.datetime.strptime(date, "%Y-%m-%d").timetuple())+opt_t).strftime('%Y-%m-%d')))[0:10]

    # Take care of edge case with round_num
    if round_num <= 0:
        round_num = 0.5
    spx_data = pull_spx_options(t1, str(datetime.datetime.now())[0:10], date3, sym=sym, lim=lim, round_num=round_num)

    vgraph = graph_options_volume(t1, sym, date, stack_bars=stack_bars)
    if dplot=='False':
        ggraph = plot_SPX_options(spx_data, sym)
    else:
        ggraph = plot_SPX_options_3d(spx_data)
    # rgraph = get_resistance_lvls(t1, date2, str(datetime.datetime.now())[0:10], plot_res=plot_res)


    return ggraph, vgraph

@app.callback(
    dash.dependencies.Output('vts-graph', 'figure'),
    [dash.dependencies.Input('symbol-dropdown', "value"),
    dash.dependencies.Input('date-picker', "date"),
    dash.dependencies.Input('option-time-dropdown', "value"),
    dash.dependencies.Input('vts-dropdown', "value"),
    dash.dependencies.Input('vts-filter', "value")])
def update_vts_figure(sym, date, opt_t, pc, vfilter):

    # If the stock doesnt have weeklies / monthlys, make min time at least 3 months
    if sym not in ['$NDX.X', '$RUT.X', '$VIX.X', '$SPX.X', 'SPY']:
        opt_t = max(7.884e+6, opt_t)

    date = str(date)[0:10]
    date3 = str(ciso8601.parse_datetime(datetime.datetime.utcfromtimestamp(time.mktime(datetime.datetime.strptime(date, "%Y-%m-%d").timetuple())+opt_t).strftime('%Y-%m-%d')))[0:10]

    vdf = pullCalendarIVdf(t1, sym, str(datetime.datetime.now())[0:10], date3, filter_fd=vfilter)
    vgraph = graphCalendarIV(sym, vdf, pc)

    return vgraph

@app.callback(
    dash.dependencies.Output('options3-graph', 'figure'),
    [dash.dependencies.Input('symbol-dropdown', "value"),
    dash.dependencies.Input('start-date', "date"),
    dash.dependencies.Input('end-date', "date"),
    dash.dependencies.Input('mod-dropdown', "value"),
    dash.dependencies.Input('stack-bars2', "value")])
def update_options3(sym, date1, date2, modifier, stack_bars):

    date1 = str(date1)[0:10]
    date2 = str(date2)[0:10]

    ograph = graph_options_oi(t1, sym, date1, date2, modifier=modifier, stack_bars=stack_bars)
